let view='<div class="city-container" style="z-index: 9999; ">\n' +
    '  <div class="city">\n' +
    ...
    '  </div>\n' +
    '</div>'
$('body').append(view).on('click', '.letter a', function () {
    var s = $(this).html();
    $('.city-container').scrollTop($('#' + s + '1').offset().top+$('.city-container').scrollTop());
});
...
function selectCity(obj){
    ...
    function callback(){
        $('.city-container').hide()
        $(obj).text(this.innerText)
        $('.city-container p').unbind('click',callback)
    }
    $('.city-container p').bind('click',callback)
}